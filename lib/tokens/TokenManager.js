...
class TokenManager {
  constructor(rDefault, jar, sessionUrl) {
    this._reqDefault = rDefault;
    this._jar = jar;
  ...
  _renew() {
    if (!this._isTokenRenewing) {
      ...
      // Create agent that keeps no connection alive and uses only 1 socket
      let protocol;
      if (this._sessionUrl.match(/^http:/)) {
        protocol = require('http');
      } else {
        protocol = require('https'); // default to https
      }
      var tokenAgent = new protocol.Agent({
        keepAlive: false,
        maxSockets: 1
      });
      let defaults = {};
      defaults = Object.assign(defaults, this._reqDefault);
      defaults.agent = tokenAgent;
      if (defaults.headers === undefined) {
        // if there are custom client used there will be no default request settings.
        defaults.headers = {'Connection': 'closed'};
      } else {
        defaults.headers['Connection'] = 'closed';
      }
      let tokenRequest = require('request').defaults(defaults);
      this._getToken(boundedTokenReceived, tokenRequest);
    }
